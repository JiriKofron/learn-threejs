...
const create = () => {
  ...
  const p2 = Point(500, 200)
  const p3 = Point(100, 400)
  const p4 = Point(300, 300)

  const s1 = Segment(p1, p2)
  const s2 = Segment(p1, p3)
  const s3 = Segment(p1, p4)
  const s4 = Segment(p2, p3)

  graph.value = Graph([p1, p2, p3, p4], [s1, s2, s3, s4]);
  graph.value.draw(ctx.value)
}

const addRandomPoint = () => {
  graph.value.tryAddPoint(
...
const removeRandomPoint = () => {
  if(graph.value.segments.length === 0) {
    console.log('no segments')
    return
  }

  const index = Math.floor(Math.random() * graph.value.points.length)
  graph.value.removePoint(graph.value.points[index])
...
const addRandomSegment = () => {
  const index1 = Math.floor(Math.random() * graph.value.points.length)
  const index2 = Math.floor(Math.random() * graph.value.points.length)

  if(index1 !== index2) {
    graph.value.tryAddSegment(
        Segment(graph.value.points[index1], graph.value.points[index2])
    )
  }
...
const removeRandomSegment = () => {
  ...
  const index = Math.floor(Math.random() * graph.value.segments.length)
  graph.value.removeSegment(graph.value.segments[index])
